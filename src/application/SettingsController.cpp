...
void SettingsController::init(InputManager& manager, StateInfo& info) {
    ...
    String interval = _info.config[LOGGING_INTERVAL_ID];
    if (interval.toInt() < 0) {
        interval = application::GlobalSettings[LOGGING_INTERVAL_ID];
    }
    dyno_assert(interval.toInt() > -1);
    _interval = static_cast<uint32_t>(interval.toInt());
    _initializeIntervalSettings(interval);
    ...
    UIEventHandler::instance().addEvent(
        [this, self]() {
            ...
            _view->init();
            _buttonCallbackPairs[_inFocus].first->focus();
        }
    );
}
...
void SettingsController::_initializeInputSettings(uint8_t id) {
    ...
    callback_t callback = std::bind(&SettingsController::_handleInputStringSelected, this, id, _1);
...
void SettingsController::_initializeOutputSettings(uint8_t id) {
    ...
    callback_t callback = std::bind(&SettingsController::_handleOutputStringSelected, this, id, _1);
    _buttonCallbackPairs.push_back(std::make_pair(button, callback));
    ...
    for (int i = 0; i < NUM_VALUE_IDS; i++) {
        ...
        callback_t callback = std::bind(&SettingsController::_handleValueIdSelected, this, vid, _1);
        _buttonCallbackPairs.push_back(std::make_pair(button, callback));
    }
}

void SettingsController::_initializeIntervalSettings(String& interval) {
    std::shared_ptr<UIButton> button = _view->buildIntervalButton(interval);
    callback_t callback = std::bind(&SettingsController::_handleIntervalSelected, this, _1);
    _buttonCallbackPairs.push_back(std::make_pair(button, callback));
}
...
SettingsController::CallbackStatus SettingsController::_handleValueIdSelected(application::ValueId id, input_data_t d) {
    if (d) {
        ...
        DEBUG_SERIAL_LN("Value id selected: " + String(id));
    }
