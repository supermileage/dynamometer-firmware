...
SessionController::SessionController(ApplicationContext &context, TFT_eSPI &display, SensorForce &force, SensorOptical &optical, BpmControl &bpm) : ControllerBase(context, display), _force(force), _optical(optical), _bpm(bpm)
{
...
std::function<String(void)> SessionController::_getValueLogger(application::ValueId id)
{
    ...
    case Force:
        return [this]()
        { return String(_force.getForce()); };
    case AngularVelocity:
        return [this]()
        { return String(_optical.getAngularVelocity()); };
    case AngularAccel:
