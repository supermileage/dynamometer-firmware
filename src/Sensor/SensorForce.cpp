...
#define MAX_READ_VALUE 977
        ...
        void SensorForce::handle() {
            if(millis() > _lastReadTime + READ_INTERVAL) {
                ...
                Serial.println(String(int_force));
                _force = ((float)(int_force < MIN_READ_VALUE ? 0 : int_force - MIN_READ_VALUE) / (MAX_READ_VALUE - MIN_READ_VALUE)) * 25;
                Serial.println(String("Force Reading: " ) + String(_force) + String(" lbf"));
            }
        }
